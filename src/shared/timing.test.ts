...
import { timingLabel, addDays, measurementDate } from './timing.js'
...
describe('measurementDate', () => {
  ...
  it('derives the date from the anchored application actual + offset', () => {
    expect(measurementDate({ applicationRef: 'A', daysAfter: 14 }, actuals)).toBe('2026-05-15')
    expect(measurementDate({ applicationRef: 'B', daysAfter: 0 }, actuals)).toBe('2026-05-20')
  })
  it('is empty when unanchored or the application has no recorded date', () => {
    expect(measurementDate({ applicationRef: '', daysAfter: null }, actuals)).toBe('')
    expect(measurementDate({ applicationRef: 'C', daysAfter: 7 }, actuals)).toBe('')
  })
})
